...
impl PySslContext {
    ...
    fn load_cert_chain(
        ...
        password: OptionalOption<Either<PyStringRef, PyCallable>>,
        vm: &VirtualMachine,
    ) -> PyResult<()> {
...
impl PySslSocket {
    fn stream(&self) -> impl std::ops::Deref<Target = ssl::SslStream<PySocketRef>> + '_ {
        ...
        PyRwLockReadGuard::map(s, |s| unsafe { std::mem::transmute(s) })
    }
    fn stream_mut(&self) -> impl std::ops::DerefMut<Target = ssl::SslStream<PySocketRef>> + '_ {
        let s = self.stream.write();
        // SAFETY: SslStreamBuilder is just a wrapper around SslStream
        PyRwLockWriteGuard::map(s, |s| unsafe { std::mem::transmute(s) })
    }
